...
import { Glob } from "@numasec/shared/util/glob"
...
function systemManagedConfigDir(): string {
  switch (process.platform) {
    case "darwin":
      return "/Library/Application Support/numasec"
    case "win32":
      return path.join(process.env.ProgramData || "C:\\ProgramData", "numasec")
    default:
      return "/etc/numasec"
  }
}
...
export function managedConfigDir() {
  return process.env.NUMASEC_TEST_MANAGED_CONFIG_DIR || systemManagedConfigDir()
}

const managedDir = managedConfigDir()

const MANAGED_PLIST_DOMAIN = "ai.numasec.managed"

// Keys injected by macOS/MDM into the managed plist that are not Numasec config
...
/**
 * Parse raw JSON (from plutil conversion of a managed plist) into Numasec config.
...
async function loadCommand(dir: string) {
  ...
  for (const item of await Glob.scan("{command,commands}/**/*.md", {
    ...
    const patterns = ["/.numasec/command/", "/.numasec/commands/", "/command/", "/commands/"]
...
async function loadAgent(dir: string) {
  ...
  for (const item of await Glob.scan("{agent,agents}/**/*.md", {
    ...
    const patterns = ["/.numasec/agent/", "/.numasec/agents/", "/agent/", "/agents/"]
    const file = rel(item, patterns) ?? path.basename(item)
...
export const Keybinds = z
  .object({
    ...
    operation_list: z.string().optional().default("<leader>o").describe("List operations"),
...
export const Server = z
  .object({
    ...
    mdnsDomain: z.string().optional().describe("Custom domain name for mDNS service (default: numasec.local)"),
...
export const Info = z
  .object({
    ...
    server: Server.optional().describe("Server configuration for numasec serve and web commands"),
    command: z
      .record(z.string(), Command)
      .optional()
      .describe("Command configuration, see https://numasec.ai/docs/commands"),
...
export class Service extends Context.Service<Service, Interface>()("@numasec/Config") {}

function globalConfigFile() {
  const candidates = ["numasec.jsonc", "numasec.json", "config.json"].map((file) =>
  ...
  Effect.gen(function* () {
    ...
    const loadConfig = Effect.fnUntraced(function* (
      ...
      if (parsed.success) {
        if (!parsed.data.$schema && isFile) {
          parsed.data.$schema = "https://numasec.ai/config.json"
    ...
    const loadGlobal = Effect.fnUntraced(function* () {
      let result: Info = pipe(
        ...
        mergeDeep(yield* loadFile(path.join(Global.Path.config, "numasec.json"))),
        mergeDeep(yield* loadFile(path.join(Global.Path.config, "numasec.jsonc"))),
      )
        ...
        yield* Effect.promise(() =>
          import(pathToFileURL(legacy).href, { with: { type: "toml" } })
            .then(async (mod) => {
              ...
              result["$schema"] = "https://numasec.ai/config.json"
    ...
    const setupConfigDir = Effect.fnUntraced(function* (dir: string) {
      ...
      yield* npmSvc.install(dir, {
        add: ["@numasec/plugin" + (InstallationLocal ? "" : "@" + InstallationVersion)],
      })
    })
    ...
    const loadInstanceState = Effect.fn("Config.loadInstanceState")(function* (ctx: InstanceContext) {
      ...
      const scope = Effect.fnUntraced(function* (source: string) {
        ...
        if (source === "NUMASEC_CONFIG_CONTENT") return "local"
      ...
      for (const [key, value] of Object.entries(auth)) {
        if (value.type === "wellknown") {
          ...
          if (!remoteConfig.$schema) remoteConfig.$schema = "https://numasec.ai/config.json"
          const source = `${url}/.well-known/numasec`
      ...
      if (Flag.NUMASEC_CONFIG) {
        yield* merge(Flag.NUMASEC_CONFIG, yield* loadFile(Flag.NUMASEC_CONFIG))
        log.debug("loaded custom config", { path: Flag.NUMASEC_CONFIG })
      }

      if (!Flag.NUMASEC_DISABLE_PROJECT_CONFIG) {
        for (const file of yield* Effect.promise(() =>
          ConfigPaths.projectFiles("numasec", ctx.directory, ctx.worktree),
        )) {
      ...
      if (Flag.NUMASEC_CONFIG_DIR) {
        log.debug("loading config from NUMASEC_CONFIG_DIR", { path: Flag.NUMASEC_CONFIG_DIR })
      }

      const deps: Fiber.Fiber<void, never>[] = []

      for (const dir of unique(directories)) {
        if (dir.endsWith(".numasec") || dir === Flag.NUMASEC_CONFIG_DIR) {
          for (const file of ["numasec.json", "numasec.jsonc"]) {
      ...
      if (process.env.NUMASEC_CONFIG_CONTENT) {
        const source = "NUMASEC_CONFIG_CONTENT"
        const next = yield* loadConfig(process.env.NUMASEC_CONFIG_CONTENT, {
          dir: ctx.directory,
          source,
        })
      ...
      if (Flag.NUMASEC_PERMISSION) {
        result.permission = mergeDeep(result.permission ?? {}, JSON.parse(Flag.NUMASEC_PERMISSION))
      }
