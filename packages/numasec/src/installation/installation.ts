...
export const USER_AGENT = `numasec/${InstallationChannel}/${InstallationVersion}/${Flag.NUMASEC_CLIENT}`
...
export class Service extends Context.Service<Service, Interface>()("@numasec/Installation") {}
  ...
  Layer.effect(
    ...
    Effect.gen(function* () {
      ...
      const getBrewFormula = Effect.fnUntraced(function* () {
        const tapFormula = yield* text(["brew", "list", "--formula", "numasec/tap/numasec"])
        if (tapFormula.includes("numasec")) return "numasec/tap/numasec"
      ...
      const upgradeImpl = Effect.fn("Installation.upgrade")(function* (m: Method, target: string) {
        ...
        switch (m) {
          ...
          case "brew": {
            ...
            if (formula.includes("/")) {
              const tap = yield* run(["brew", "tap", "numasec/tap"], { env })
              if (tap.code !== 0) {
                result = tap
                break
              }
              const repo = yield* text(["brew", "--repo", "numasec/tap"])
              const dir = repo.trim()
              if (dir) {
