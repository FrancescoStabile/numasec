...
export async function checkPluginCompatibility(target: string, numasecVersion: string, pkg?: PluginPackage) {
  if (!semver.valid(numasecVersion) || semver.major(numasecVersion) === 0) return
  const hit = pkg ?? (await readPluginPackage(target).catch(() => undefined))
  if (!hit) return
  const engines = hit.json.engines
  if (!isRecord(engines)) return
  const range = engines.numasec
  if (typeof range !== "string") return
  if (!semver.satisfies(numasecVersion, range)) {
    throw new Error(`Plugin requires numasec ${range} but running ${numasecVersion}`)
  }
}
