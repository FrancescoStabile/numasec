...
export class Service extends Context.Service<Service, Interface>()("@numasec/Plugin") {}

// Built-in plugins that are directly imported (not installed from npm)
// External plugins from opencode-* npm packages use @opencode-ai/plugin types
// which are structurally compatible but nominally distinct from @numasec/plugin
      ...
      Effect.fn("Plugin.state")(function* (ctx) {
        ...
        const input: PluginInput = {
          ...
          experimental_workspace: {
            register(_type: string, _adaptor: unknown) {
              // workspace adaptors removed
            },
          },
          get serverUrl(): URL {
        ...
        const plugins = Flag.NUMASEC_PURE ? [] : (cfg.plugin_origins ?? [])
        if (Flag.NUMASEC_PURE && cfg.plugin_origins?.length) {
