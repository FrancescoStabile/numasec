...
const Headers = Schema.Struct({
  ...
  "x-numasec-directory": Schema.optional(Schema.String),
})
...
export namespace ExperimentalHttpApiServer {
  ...
  class Authorization extends HttpApiMiddleware.Service<Authorization>()("@numasec/ExperimentalHttpApiAuthorization", {
    error: Unauthorized,
    security: {
  ...
  const auth = Layer.succeed(
    ...
    Authorization.of({
      basic: (effect, { credential }) =>
        Effect.gen(function* () {
          if (!Flag.NUMASEC_SERVER_PASSWORD) return yield* effect

          const user = Flag.NUMASEC_SERVER_USERNAME ?? "numasec"
          if (credential.username !== user) {
            return yield* new Unauthorized({ message: "Unauthorized" })
          }
          if (Redacted.value(credential.password) !== Flag.NUMASEC_SERVER_PASSWORD) {
            return yield* new Unauthorized({ message: "Unauthorized" })
          }
          return yield* effect
        }),
    ...
    Effect.gen(function* () {
      return (effect) =>
        Effect.gen(function* () {
          ...
          const raw = query.directory || headers["x-numasec-directory"] || process.cwd()
