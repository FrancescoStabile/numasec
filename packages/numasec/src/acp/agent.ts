...
export namespace ACP {
  ...
  export async function init({ sdk: _sdk }: { sdk: NumasecClient }) {
    return {
  ...
  export class Agent implements ACPAgent {
    ...
    async initialize(params: InitializeRequest): Promise<InitializeResponse> {
      ...
      const authMethod: AuthMethod = {
        description: "Run `numasec auth login` in the terminal",
        name: "Login with numasec",
        id: "numasec-login",
      }
    ...
    private async processMessage(message: SessionMessageResponse) {
      ...
      for (const part of message.parts) {
        if (part.type === "tool") {
          ...
          // Numasec stores files internally as { type: "file", url, filename, mime }.
  ...
  async function defaultModel(config: ACPConfig, cwd?: string): Promise<{ providerID: ProviderID; modelID: ModelID }> {
    ...
    const numasecProvider = providers.find((p) => p.id === "numasec")
    if (numasecProvider) {
      if (numasecProvider.models["big-pickle"]) {
        return { providerID: ProviderID.numasec, modelID: ModelID.make("big-pickle") }
      }
      const [best] = Provider.sort(Object.values(numasecProvider.models))
      if (best) {
        return {
