...
const cli = yargs(args)
  ...
  .middleware(async (opts) => {
    ...
    process.env.AGENT = "1"
    process.env.NUMASEC = "1"
    process.env.NUMASEC_PID = String(process.pid)

    Log.Default.info("numasec", {
      version: InstallationVersion,
      args: process.argv.slice(2),
    })

    const marker = path.join(Global.Path.data, "numasec.db")
