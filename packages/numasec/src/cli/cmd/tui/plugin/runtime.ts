...
function createThemeInstaller(
  ...
  return async (file) => {
    ...
    const local_dir =
      path.basename(source_dir) === ".numasec"
        ? path.join(source_dir, "themes")
        : path.join(source_dir, ".numasec", "themes")
...
function defaultPluginOrigin(state: RuntimeState, spec: string): ConfigPlugin.Origin {
  return {
    spec,
    scope: "local",
    source: state.api.state.path.config || path.join(state.directory, ".numasec", "tui.json"),
  }
}
...
export namespace TuiPluginRuntime {
  ...
  async function load(input: { api: Api; config: TuiConfig.Info }) {
    ...
    runtime = next
    try {
      const records = Flag.NUMASEC_PURE ? [] : (config.plugin_origins ?? [])
      if (Flag.NUMASEC_PURE && config.plugin_origins?.length) {
