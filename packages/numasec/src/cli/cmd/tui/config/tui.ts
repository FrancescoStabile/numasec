...
export namespace TuiConfig {
  ...
  export class Service extends Context.Service<Service, Interface>()("@numasec/TuiConfig") {}
  ...
  function customPath() {
    return Flag.NUMASEC_TUI_CONFIG
  }
  ...
  async function loadState(ctx: { directory: string }) {
    let projectFiles = Flag.NUMASEC_DISABLE_PROJECT_CONFIG ? [] : await ConfigPaths.projectFiles("tui", ctx.directory)
    ...
    projectFiles = Flag.NUMASEC_DISABLE_PROJECT_CONFIG ? [] : await ConfigPaths.projectFiles("tui", ctx.directory)
    ...
    const dirs = unique(directories).filter((dir) => dir.endsWith(".numasec") || dir === Flag.NUMASEC_CONFIG_DIR)

    for (const dir of dirs) {
      if (!dir.endsWith(".numasec") && dir !== Flag.NUMASEC_CONFIG_DIR) continue
  ...
  async function load(text: string, configFilepath: string): Promise<Info> {
    ...
    // (mirroring the old numasec.json shape) still get their settings applied.
