...
const TUI_SCHEMA_URL = "https://numasec.ai/tui.json"
...
/**
 * Migrates tui-specific keys (theme, keybinds, tui) from numasec.json files
 * into dedicated tui.json files. Migration is performed per-directory and
 * skips only locations where a tui.json already exists.
 */
export async function migrateTuiConfig(input: MigrateInput) {
  const numasec = await numasecFiles(input)
  for (const file of numasec) {
...
async function numasecFiles(input: { directories: string[]; cwd: string }) {
  const project = Flag.NUMASEC_DISABLE_PROJECT_CONFIG ? [] : await ConfigPaths.projectFiles("numasec", input.cwd)
  const files = [...project, ...ConfigPaths.fileInDirectory(Global.Path.config, "numasec")]
  for (const dir of unique(input.directories)) {
    files.push(...ConfigPaths.fileInDirectory(dir, "numasec"))
  }
  if (Flag.NUMASEC_CONFIG) files.push(Flag.NUMASEC_CONFIG)
