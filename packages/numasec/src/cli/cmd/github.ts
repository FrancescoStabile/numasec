...
const AGENT_USERNAME = "numasec-agent[bot]"
const AGENT_REACTION = "eyes"
const WORKFLOW_FILE = ".github/workflows/numasec.yml"
      ...
      async fn() {
        {
          ...
          function printNextSteps() {
            ...
            prompts.outro(
              [
                ...
                "",
                "   Learn more about the GitHub agent - https://numasec.ai/docs/github/#usage-examples",
              ].join("\n"),
            )
          }
          ...
          async function installGitHubApp() {
            ...
            // Open browser
            const url = "https://github.com/apps/numasec-agent"
            const command =
...
jobs:
  numasec:
    if: |
      ...
      contains(github.event.comment.body, ' /numasec') ||
      startsWith(github.event.comment.body, '/numasec')
...
export const GithubRunCommand = cmd({
  ...
  async handler(args) {
    await bootstrap(process.cwd(), async () => {
      ...
      const shareBaseUrl = isMock ? "https://dev.numasec.ai" : "https://numasec.ai"
      ...
      try {
        ...
        console.log("numasec session", session.id)
      ...
      async function getUserPrompt() {
        ...
        const mentions = (process.env["MENTIONS"] || "/numasec,/oc")
          .split(",")
      ...
      async function chat(message: string, files: PromptFiles = []) {
        console.log("Sending message to numasec...")
        ...
        return AppRuntime.runPromise(
          Effect.gen(function* () {
            ...
            const result = yield* prompt.prompt({
              ...
              // agent is omitted - server will use default_agent from config or fall back to "security"
              parts: [
                {
                  id: PartID.ascending(),
                  type: "text",
      ...
      async function getOidcToken() {
        try {
          return await core.getIDToken("numasec-github-action")
        } catch (error) {
      ...
      function generateBranchName(type: "issue" | "pr" | "schedule" | "dispatch") {
        ...
        if (type === "schedule" || type === "dispatch") {
          ...
          return `numasec/${type}-${hex}-${timestamp}`
        }
        return `numasec/${type}${issueId}-${timestamp}`
      }
      ...
      function footer(opts?: { image?: boolean }) {
        const image = (() => {
          ...
          return `<a href="${shareBaseUrl}/s/${shareId}"><img width="200" alt="${titleAlt}" src="https://social-cards.sst.dev/numasec-share/${title64}.png?model=${providerID}/${modelID}&version=${session.version}&id=${shareId}" /></a>\n`
        })()
        const shareUrl = shareId ? `[numasec session](${shareBaseUrl}/s/${shareId})&nbsp;&nbsp;|&nbsp;&nbsp;` : ""
