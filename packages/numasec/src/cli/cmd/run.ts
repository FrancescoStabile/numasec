...
import { createNumasecClient, type NumasecClient, type ToolPart } from "@numasec/sdk/v2"
...
export const RunCommand = cmd({
  command: "run [message..]",
  describe: "run numasec with a message",
  builder: (yargs: Argv) => {
    return yargs
      .positional("message", {
      ...
      .option("attach", {
        type: "string",
        describe: "attach to a running numasec server (e.g., http://localhost:4096)",
      })
      .option("password", {
        alias: ["p"],
        type: "string",
  ...
  handler: async (args) => {
    ...
    async function session(sdk: NumasecClient) {
    ...
    await bootstrap(process.cwd(), async () => {
      ...
      const sdk = createNumasecClient({ baseUrl: "http://numasec.internal", fetch: fetchFn })
      await execute(sdk)
    })
  },
})
