...
const FILES = [
  "AGENTS.md",
  ...(Flag.NUMASEC_DISABLE_CLAUDE_CODE_PROMPT ? [] : ["CLAUDE.md"]),
...
export namespace Instruction {
  ...
  export class Service extends Context.Service<Service, Interface>()("@numasec/Instruction") {}
    ...
    Layer.effect(
      ...
      Effect.gen(function* () {
        ...
        const relative = Effect.fnUntraced(function* (instruction: string) {
          ...
          if (!Flag.NUMASEC_CONFIG_DIR) {
            log.warn(
              `Skipping relative instruction "${instruction}" - no NUMASEC_CONFIG_DIR set while project config is disabled`,
            )
            return []
          }
          return yield* fs
            .globUp(instruction, Flag.NUMASEC_CONFIG_DIR, Flag.NUMASEC_CONFIG_DIR)
            .pipe(Effect.catch(() => Effect.succeed([] as string[])))
        })
        ...
        const systemPaths = Effect.fn("Instruction.systemPaths")(function* () {
          ...
          if (!Flag.NUMASEC_DISABLE_PROJECT_CONFIG) {
            ...
            // .numasec.md is always loaded alongside other instruction files.
            for (const file of NUMASEC_FILES) {
              const matches = yield* fs.findUp(file, Instance.directory, Instance.worktree)
              matches.forEach((item) => paths.add(path.resolve(item)))
            }
            const dotDir = path.join(Instance.directory, ".numasec")
            for (const file of NUMASEC_FILES) {
        ...
        const find = Effect.fn("Instruction.find")(function* (dir: string) {
          for (const file of [...FILES, ...NUMASEC_FILES]) {
