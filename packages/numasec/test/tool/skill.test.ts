...
describe("tool.skill", () => {
  it.live("description lists skill location URL", () =>
    provideTmpdirInstance(
      (dir) =>
        Effect.gen(function* () {
          const skill = path.join(dir, ".numasec", "skill", "tool-skill")
...
`,
          ...
          process.env.NUMASEC_TEST_HOME = dir
          ...
          const desc =
            (yield* registry.tools({
              providerID: "numasec" as any,
              modelID: "gpt-5" as any,
              agent: { name: "security", mode: "primary", permission: [], options: {} },
      ...
      (dir) =>
        Effect.gen(function* () {
          for (const [name, description] of [
            ...
            const skill = path.join(dir, ".numasec", "skill", name)
            yield* Effect.promise(() =>
              Bun.write(
                path.join(skill, "SKILL.md"),
                `---
...
`,
          ...
          const agent = { name: "security", mode: "primary" as const, permission: [], options: {} }
          const registry = yield* ToolRegistry.Service
