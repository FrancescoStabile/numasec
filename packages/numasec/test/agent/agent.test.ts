...
test("security agent has correct default properties", async () => {
  ...
  await Instance.provide({
    directory: tmp.path,
    fn: async () => {
      const build = await load(tmp.path, (svc) => svc.get("security"))
...
test("custom agent config overrides native agent properties", async () => {
  ...
  await Instance.provide({
    ...
    fn: async () => {
      ...
      expect(build?.description).toBe("Custom security agent")
...
test("agent prompt can be set from config", async () => {
  await using tmp = await tmpdir({
    config: {
      agent: {
        security: { prompt: "Custom system prompt" },
      },
    },
  })
  await Instance.provide({
...
test("defaultAgent returns plan when security is disabled and default_agent not set", async () => {
  await using tmp = await tmpdir({
    config: {
      agent: {
