  ...
  test("does not loop empty assistant turns for a simple reply", async () => {
    ...
    const server = Bun.serve({
      ...
      fetch(req) {
        ...
        calls++
        return new Response(chat("packages/numasec/src/session/processor.ts"), {
          status: 200,
          ...
          run(
            Effect.gen(function* () {
              ...
              const other = yield* prompt.prompt({
                sessionID: session.id,
                agent: "security",
                model: { providerID: ProviderID.make("numasec"), modelID: ModelID.make("kimi-k2.5-free") },
                noReply: true,
      ...
      fn: () =>
        run(
          Effect.gen(function* () {
            ...
            expect(NamedError.Unknown.isInstance(err)).toBe(true)
            if (NamedError.Unknown.isInstance(err)) {
              expect(err.data.message).toContain("security")
            }
          }),
        ),
    })
