    ...
    provideTmpdirServer(
      Effect.fnUntraced(function* ({ llm }) {
        ...
        const tool = yield* Effect.promise(async () => {
          ...
          while (Date.now() < end) {
            const msgs = await Effect.runPromise(MessageV2.filterCompactedEffect(chat.id))
            const assistant = msgs.findLast((item) => item.info.role === "assistant" && item.info.agent === "security")
  ...
  () =>
    provideTmpdirServer(
      Effect.fnUntraced(function* ({ llm }) {
        ...
        const exit = yield* prompt.shell({ sessionID: chat.id, agent: "security", command: "echo hi" }).pipe(Effect.exit)
        expect(Exit.isFailure(exit)).toBe(true)
        if (Exit.isFailure(exit)) {
          expect(Cause.squash(exit.cause)).toBeInstanceOf(Session.BusyError)
        }
        ...
        (_dir) =>
          Effect.gen(function* () {
            ...
            const fiber = yield* prompt
              .shell({ sessionID: chat.id, agent: "security", command: "printf first && sleep 0.2 && printf second" })
              .pipe(Effect.forkChild)
      ...
      provideTmpdirInstance(
        (_dir) =>
          Effect.gen(function* () {
            ...
            const sh = yield* prompt
              .shell({ sessionID: chat.id, agent: "security", command: "trap '' TERM; sleep 30" })
              .pipe(Effect.forkChild)
            yield* Effect.sleep(50)
        ...
        (_dir) =>
          Effect.gen(function* () {
            ...
            const exit = yield* prompt
              .shell({ sessionID: chat.id, agent: "security", command: "echo hi" })
              .pipe(Effect.exit)
            expect(Exit.isFailure(exit)).toBe(true)
            if (Exit.isFailure(exit)) {
              expect(Cause.squash(exit.cause)).toBeInstanceOf(Session.BusyError)
