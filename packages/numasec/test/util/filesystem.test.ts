...
describe("filesystem", () => {
  ...
  describe("findUp()", () => {
    ...
    test("rootFirst preserves json then jsonc order per directory", async () => {
      ...
      await fs.writeFile(path.join(tmp.path, "numasec.json"), "{}", "utf-8")
      await fs.writeFile(path.join(tmp.path, "numasec.jsonc"), "{}", "utf-8")
      await fs.writeFile(path.join(project, "numasec.json"), "{}", "utf-8")
      await fs.writeFile(path.join(project, "numasec.jsonc"), "{}", "utf-8")

      const result = await Filesystem.findUp(["numasec.json", "numasec.jsonc"], nested, tmp.path, {
        rootFirst: true,
      })

      expect(result).toEqual([
        path.join(tmp.path, "numasec.json"),
        path.join(tmp.path, "numasec.jsonc"),
        path.join(project, "numasec.json"),
        path.join(project, "numasec.jsonc"),
      ])
    })
  })
