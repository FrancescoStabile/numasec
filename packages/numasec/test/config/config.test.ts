...
import { EffectFlock } from "@numasec/shared/util/effect-flock"
...
const managedConfigDir = process.env.NUMASEC_TEST_MANAGED_CONFIG_DIR!
...
async function writeManagedSettings(settings: object, filename = "numasec.json") {
  await fs.mkdir(managedConfigDir, { recursive: true })
  await Filesystem.write(path.join(managedConfigDir, filename), JSON.stringify(settings))
}

async function writeConfig(dir: string, config: object, name = "numasec.json") {
...
test("ignores legacy tui keys in numasec config", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      await writeConfig(dir, {
        $schema: "https://numasec.ai/config.json",
  ...
  try {
    ...
    await Instance.provide({
      ...
      fn: async () => {
        ...
        const content = await Filesystem.readText(path.join(tmp.path, "numasec.json"))
...
test("throws error for invalid JSON", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      await Filesystem.write(path.join(dir, "numasec.json"), "{ invalid json }")
    },
  })
  await Instance.provide({
    directory: tmp.path,
...
test("loads config from .numasec directory", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const numasecDir = path.join(dir, ".numasec")
      await fs.mkdir(numasecDir, { recursive: true })
      const agentDir = path.join(numasecDir, "agent")
...
test("loads agents from .numasec/agents (plural)", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const numasecDir = path.join(dir, ".numasec")
      await fs.mkdir(numasecDir, { recursive: true })

      const agentsDir = path.join(numasecDir, "agents")
...
test("loads commands from .numasec/command (singular)", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const numasecDir = path.join(dir, ".numasec")
      await fs.mkdir(numasecDir, { recursive: true })

      const commandDir = path.join(numasecDir, "command")
...
test("loads commands from .numasec/commands (plural)", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const numasecDir = path.join(dir, ".numasec")
      await fs.mkdir(numasecDir, { recursive: true })

      const commandsDir = path.join(numasecDir, "commands")
...
test("does not try to install dependencies in read-only NUMASEC_CONFIG_DIR", async () => {
  if (process.platform === "win32") return
  ...
  const prev = process.env.NUMASEC_CONFIG_DIR
  process.env.NUMASEC_CONFIG_DIR = tmp.extra
  ...
  try {
    ...
    if (prev === undefined) delete process.env.NUMASEC_CONFIG_DIR
    else process.env.NUMASEC_CONFIG_DIR = prev
  }
})

test("installs dependencies in writable NUMASEC_CONFIG_DIR", async () => {
  await using tmp = await tmpdir<string>({
    init: async (dir) => {
...
test("resolves scoped npm plugins in config", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      await Filesystem.write(
        path.join(dir, "numasec.json"),
        JSON.stringify({ $schema: "https://numasec.ai/config.json", plugin: ["@scope/plugin"] }, null, 2),
      )
    },
  })
...
test("merges plugin arrays from global and local configs", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      // Create a nested project structure with local .numasec config
      const projectDir = path.join(dir, "project")
      const numasecDir = path.join(projectDir, ".numasec")
      await fs.mkdir(numasecDir, { recursive: true })
      ...
      // Local .numasec config with different plugins
      await Filesystem.write(
        path.join(numasecDir, "numasec.json"),
        JSON.stringify({
          $schema: "https://numasec.ai/config.json",
...
test("deduplicates duplicate plugins from global and local configs", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      // Local .numasec config with some overlapping plugins
      await Filesystem.write(
        path.join(numasecDir, "numasec.json"),
        JSON.stringify({
          $schema: "https://numasec.ai/config.json",
...
test("keeps plugin origins aligned with merged plugin list", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      const local = path.join(project, ".numasec")
...
// Note: preload.ts sets NUMASEC_TEST_MANAGED_CONFIG which Global.Path.managedConfig uses
...
test("local .numasec config can override MCP from project config", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      // Local .numasec directory config enables it
      const numasecDir = path.join(dir, ".numasec")
      await fs.mkdir(numasecDir, { recursive: true })
      await Filesystem.write(
        path.join(numasecDir, "numasec.json"),
      ...
      () =>
        Config.Service.use((svc) =>
          Effect.gen(function* () {
            ...
            expect(fetchedUrl).toBe("https://example.com/.well-known/numasec")
...
describe("resolvePluginSpec", () => {
  test("keeps package specs unchanged", async () => {
    await using tmp = await tmpdir()
    const file = path.join(tmp.path, "numasec.json")
    expect(await ConfigPlugin.resolvePluginSpec("oh-my-numasec@2.4.3", file)).toBe("oh-my-numasec@2.4.3")
...
describe("deduplicatePluginOrigins", () => {
  ...
  test("keeps path plugins separate from package plugins", () => {
    const plugins = ["oh-my-numasec@2.4.3", "file:///project/.numasec/plugin/oh-my-numasec.js"]
  ...
  test("deduplicates direct path plugins by exact spec", () => {
    const plugins = ["file:///project/.numasec/plugin/demo.ts", "file:///project/.numasec/plugin/demo.ts"]

    const result = dedupe(plugins)

    expect(result).toEqual(["file:///project/.numasec/plugin/demo.ts"])
  })
  ...
  test("loads auto-discovered local plugins as file urls", async () => {
    await using tmp = await tmpdir({
      init: async (dir) => {
        const projectDir = path.join(dir, "project")
        const numasecDir = path.join(projectDir, ".numasec")
        const pluginDir = path.join(numasecDir, "plugin")
        await fs.mkdir(pluginDir, { recursive: true })
...
describe("NUMASEC_DISABLE_PROJECT_CONFIG", () => {
  test("skips project config files when flag is set", async () => {
    const originalEnv = process.env["NUMASEC_DISABLE_PROJECT_CONFIG"]
    process.env["NUMASEC_DISABLE_PROJECT_CONFIG"] = "true"
    ...
    try {
      ...
      if (originalEnv === undefined) {
        delete process.env["NUMASEC_DISABLE_PROJECT_CONFIG"]
      } else {
        process.env["NUMASEC_DISABLE_PROJECT_CONFIG"] = originalEnv
      }
    }
  })

  test("skips project .numasec/ directories when flag is set", async () => {
    const originalEnv = process.env["NUMASEC_DISABLE_PROJECT_CONFIG"]
    process.env["NUMASEC_DISABLE_PROJECT_CONFIG"] = "true"
    ...
    try {
      await using tmp = await tmpdir({
        init: async (dir) => {
          // Create a .numasec directory with a command
          const numasecDir = path.join(dir, ".numasec", "command")
          await fs.mkdir(numasecDir, { recursive: true })
          await Filesystem.write(path.join(numasecDir, "test-cmd.md"), "# Test Command\nThis is a test command.")
        },
      })
      await Instance.provide({
        directory: tmp.path,
  ...
  test("skips relative instructions with warning when flag is set but no config dir", async () => {
    const originalDisable = process.env["NUMASEC_DISABLE_PROJECT_CONFIG"]
    ...
    try {
      ...
      if (originalDisable === undefined) {
        ...
        process.env["NUMASEC_DISABLE_PROJECT_CONFIG"] = originalDisable
      }
  ...
  test("NUMASEC_CONFIG_DIR still works when flag is set", async () => {
    const originalDisable = process.env["NUMASEC_DISABLE_PROJECT_CONFIG"]
    const originalConfigDir = process.env["NUMASEC_CONFIG_DIR"]
    ...
    try {
      ...
      process.env["NUMASEC_DISABLE_PROJECT_CONFIG"] = "true"
      process.env["NUMASEC_CONFIG_DIR"] = configDirTmp.path

      await Instance.provide({
        directory: projectTmp.path,
        fn: async () => {
          const config = await load()
          // Should load from NUMASEC_CONFIG_DIR, not project
...
describe("NUMASEC_CONFIG_CONTENT token substitution", () => {
  test("substitutes {env:} tokens in NUMASEC_CONFIG_CONTENT", async () => {
    const originalEnv = process.env["NUMASEC_CONFIG_CONTENT"]
    const originalTestVar = process.env["TEST_CONFIG_VAR"]
    process.env["TEST_CONFIG_VAR"] = "test_api_key_12345"
    process.env["NUMASEC_CONFIG_CONTENT"] = JSON.stringify({
      $schema: "https://numasec.ai/config.json",
    ...
    try {
      ...
      if (originalEnv !== undefined) {
        process.env["NUMASEC_CONFIG_CONTENT"] = originalEnv
      } else {
        delete process.env["NUMASEC_CONFIG_CONTENT"]
      }
  ...
  test("substitutes {file:} tokens in NUMASEC_CONFIG_CONTENT", async () => {
    const originalEnv = process.env["NUMASEC_CONFIG_CONTENT"]
...
test("parseManagedPlist strips MDM metadata keys", async () => {
  const config = await Config.parseManagedPlist(
    JSON.stringify({
      PayloadDisplayName: "Numasec Managed",
      PayloadIdentifier: "ai.numasec.managed.test",
      PayloadType: "ai.numasec.managed",
...
test("parseManagedPlist handles empty config", async () => {
  const config = await Config.parseManagedPlist(
    JSON.stringify({ $schema: "https://numasec.ai/config.json" }),
    "test:mobileconfig",
  )
  expect(config.$schema).toBe("https://numasec.ai/config.json")
})
