...
function rewrite(request: Request, values: { directory?: string; workspace?: string }) {
  ...
  for (const [name, key] of [
    ["x-numasec-directory", "directory"],
    ["x-numasec-workspace", "workspace"],
  ] as const) {
    const value = pick(
  ...
  next.headers.delete("x-numasec-workspace")
  return next
}

export function createNumasecClient(config?: Config & { directory?: string; experimental_workspaceID?: string }) {
  if (!config?.fetch) {
  ...
  if (config?.experimental_workspaceID) {
    config.headers = {
      ...config.headers,
      "x-numasec-workspace": config.experimental_workspaceID,
    }
  }
  ...
  client.interceptors.response.use((response) => {
    ...
    if (contentType === "text/html")
      throw new Error("Request is not supported by this version of numasec Server (Server responded with text/html)")
